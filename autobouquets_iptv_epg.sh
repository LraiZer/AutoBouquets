...
# auto assigns jmx,bmx iptv bouquets with _live_ stream epg
# service references generated by AutoBouquets E2 for 28.2E.

VERSIONDATE="30 Nov 2024"

# assign a search filter to limit conversion to found matches
# ie. "MyIPTV_UK" finds bouquet _live_MyIPTV_UK_Sport.tv
...
1199=SkyAnimationHD
1289=PremierSports1,Viaplay1HD,ViaplaySports1
1290=PremierSports2,Viaplay2HD,ViaplaySports2,ViaplayExtra
1336=TNTUltimateUHD,TNTSportsUltimateUHD
1409=SkyPremiere
1633=PremierSports1Ireland
1635=PremierSports2Ireland
...
if [ -e $AUTOBOUQUETS_PATH/autobouquets.csv ]; then
	...
	IFS='
'
	echo "Updating _live_ IPTV EPG, working..."
	for BOUQUET in `ls $E2BQ_PATH/*_live_*$BOUQUET_FILTER*.tv`; do
	...
	for BOUQUET in `ls $IEPG_PATH/*_live_*$BOUQUET_FILTER*.tv`; do
		SERVICES=`grep -i '^#SERVICE.*http*' $BOUQUET`
	...
	IFS=$OIFS
	mv $IEPG_PATH/*_live_*$BOUQUET_FILTER*.tv $E2BQ_PATH
	rm -rf $IEPG_PATH
else
