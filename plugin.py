...
config.autobouquets.iptvepg = ConfigYesNo(default=True)
...
class AutoBouquetsMenu(ConfigListScreen, Screen):
	...
	def createSetup(self):
		...
		self.list.append(getConfigListEntry(_("Show Iptv EPG Converter"), config.autobouquets.iptvepg,
			_("Attempts to auto convert live UK Iptv bouquets generated by BMX or JMX to use enigma2 opentv stream EPG service references generated from the AutoBouquets E2 for 28.2E plugin")))
...
class AutoBouquetsIptvEPG(Screen):
	...
	def __init__(self, session):
		Screen.__init__(self, session)
		self.session = session
		self["action"] = Label(_("AutoBouquets Iptv EPG"))
		self["status"] = Label(_("converting bouquets"))
		self.doConvertIptvEPG()

	def doConvertIptvEPG(self):
		com = ("cd %s; /bin/busybox ash ./autobouquets_iptv_epg.sh " % (path.dirname(modules[__name__].__file__)))
		self.shcomConvertIptvEPG(com)

	def shcomConvertIptvEPG(self, com):
		try:
			global container

			def appClosed(retval):
				global container
				print("[AutoBouquets Iptv EPG Converter] FINISHED: ", retval)
				container = None
				self.convertIptvEPGcomplete()
			container = eConsoleAppContainer()
			if container.execute(com):
				raise Exception("Script Failed: " + com)
			container.appClosed.append(appClosed)
		except Exception as e:
			print("[AutoBouquets Iptv EPG Converter] FAILED: ", e)

	def convertIptvEPGcomplete(self):
		eDVBDB.getInstance().reloadBouquets()
		self.timer = eTimer()
		self.timer.callback.append(self.close(None))
		self.timer.start(5000, 1)
...
def iptvepgconverter(session, **kwargs):
	session.open(AutoBouquetsIptvEPG)


def iptvepg(menuid, **kwargs):
	if menuid in ("epg", "epg_menu",):
		return [(_("AutoBouquets IptvEPG"), iptvepgconverter, _("convert Iptv to 28.2e stream EPG"), None)]
	else:
		return []
